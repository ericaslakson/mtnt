...
(function () {
  ...
  */
  ...
  DragZoom.prototype.init_ = function (map, opt_zoomOpts) {
    ...
    this.listeners_ = [
      google.maps.event.addDomListener(document, "keydown", function (e) {
        // console.log("google.maps.event.addDomListener(document, keydown ");
        me.onKeyDown_(e);
      }),
      google.maps.event.addDomListener(document, "keyup", function (e) {
        // console.log("google.maps.event.addDomListener(document, keyup ");
        me.onKeyUp_(e);
      }),
      ...
      google.maps.event.addDomListener(document, "mousedown", function (e) {
        // added era to remove requirement for shift. this to match visjs.org network right click sweep behavior
        //console.log("google.maps.event.addDomListener(document, mousedown button number: " + e.which);
        if(e.which == 3) {
          me.pseudoKeyDown();
          //console.log("calling pseudoKeyDown and me.onMouseDown");
	  me.pseudoMouseDown_(e);
        }
        // end added era
      ...
      google.maps.event.addDomListener(document, "mouseup", function (e) {
        me.onMouseUp_(e);
        // added era to remove requirement for shift in order to match visjs.org network control behavior
        //console.log("google.maps.event.addDomListener(document, mouseup button number: " + e.which);
        if(e.which == 3) {
          //console.log("setting hotKeyDown false");
	  me.onKeyUp_(e);
        }
        // end added era
      }),
  ...
  /**
   * Added era . Handle pseudo key down. This is a pseudo replacement for onKeyDown.  
   * Show the veil if the hot key has been pressed.
   * @param {Event} e The keyboard event.
   */
  DragZoom.prototype.pseudoKeyDown = function () {
    //if (this.map_ && !this.hotKeyDown_ && this.isHotKeyDown_(e)) {
  ...
  /**
   * Pseudo mouse down. mod by era 
   * @param {Event} e The mouse event.
   */
  DragZoom.prototype.pseudoMouseDown_ = function (e) {
    if (this.map_ && this.hotKeyDown_) {
      this.mapPosn_ = getElementPosition(this.map_.getDiv());
      this.dragging_ = true;
      this.startPt_ = this.endPt_ = this.getMousePoint_(e);
  ...
  DragZoom.prototype.onMouseUp_ = function (e) {
    ...
    if (this.dragging_) {
      ...
      // added era calculate which markers are in the bnd range and select or deselect them
      // first deselect current selections if no cntldown
      if(!ctrldown) {
      ...
      //console.log("node_loc_data.length: " + node_loc_data.length);
      for(var i = 0; i < node_loc_data.length; i++) {
        if(bnds.contains(node_loc_data[i].latlng)) {
          //console.log("setting marker with name to selected: " + node_loc_data[i].name);
          markers[i].setIcon(highlightedIcon());
	  selected_marker_names.push(node_loc_data[i].name);
        } 
      }
      if(selected_marker_names.length > 0) {
        parent.window.map_win.window.className = "map";
	console.log("about to dispatch event from map multiselect");
        parent.window.EventBus.dispatch('select',parent.window.map_win.window,selected_marker_names);
      }
      // end era 
